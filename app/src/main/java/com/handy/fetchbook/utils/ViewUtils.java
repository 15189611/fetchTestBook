...
public class ViewUtils {
    ...
    public static void setBackgroundColorResId(View view, int resId) {
        view.setBackgroundColor(Utils.getApp().getResources().getColor(resId));
    }
    ...
    public static LayoutInflater getLayoutInflater() {
        return LayoutInflater.from(Utils.getApp());
    }
     ...
     * @return
     */
    public static boolean setLyaoutParms(View view, int width, int height) {
        ViewGroup.LayoutParams params = view.getLayoutParams();
        if (params != null) {
            if (params.width != width) {
                params.width = width;
     ...
     * @return
     */
    public static boolean addView(ViewGroup parent, ViewGroup.LayoutParams params, View child) {
    ...
    public static int getStatusBarHeight() {
        int result = 0;
        int resourceId = ActivityUtils.getTopActivity().getResources().getIdentifier("status_bar_height", "dimen", "android");
        if (resourceId > 0) {
            result = ActivityUtils.getTopActivity().getResources().getDimensionPixelSize(resourceId);
        }
        return result;
    }
    ...
    public static void setBackgroundGray(View view) {
        if (view != null) {
            ...
            if (drawable != null) {
                ...
                Bitmap bmpGray = getGrayBitmap(bmp);
                if (bmpGray != null) {
                    Drawable drawableGray = ImageUtils.bitmap2Drawable(bmpGray);
    ...
    public static void setStatusBarTintResource(Activity activity, int res) {
        try {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                BarUtils.setNavBarColor(activity.getWindow(), res);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public static <T> void updateList(List<T> listOriginalData, List<T> listNewData, boolean isLoadMore, String noData, String noMoreData) {
        if (listOriginalData != null) {
            if (listNewData != null && listNewData.size() > 0) // 有新数据
            {
                ...
                if (!isLoadMore) {
                    if (!ObjectUtils.isEmpty(noData)) {
                        ToastUtils.showLong(noData);
                    }
                    listOriginalData.clear();
                } else {
                    if (!ObjectUtils.isEmpty(noMoreData)) {
                        ToastUtils.showLong(noMoreData);
                    }
                }
            }
        }
