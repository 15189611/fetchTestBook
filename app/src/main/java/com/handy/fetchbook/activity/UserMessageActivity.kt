...
import kotlinx.android.synthetic.main.home_activity_system_info.allCleanMessage
import kotlinx.android.synthetic.main.home_activity_user_message.back
import kotlinx.android.synthetic.main.home_activity_user_message.rv
...
class UserMessageActivity : BaseVmActivity<HomeViewModel>() {
    ...
    override fun initView(savedInstanceState: Bundle?) {
        ...
        allCleanMessage.setOnClickListener {
            //清空所有传"message"
            mViewModel.requestNoCheck({ apiService.messageRead("message") }, success = {
    ...
    private fun getData(isRefresh: Boolean) {
        mViewModel.message(isRefresh, 1, currentPage)
    }
    ...
    override fun createObserver() {
        ...
        mViewModel.messageResult.observe(this) { resultState ->
            parseMyState(resultState, { isRefresh, model ->
                ...
                BooKLogger.d("个人信息接口成功-> total = $total -> size = ${model.items.size} -> isRefresh = $isRefresh")
