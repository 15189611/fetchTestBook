...
open class VLModuleAdapter(
            ...
            return ExtendGridLayoutHelper(spanCount).also {
                ...
                spanSizeLookup = object : ExtendGridLayoutHelper.SpanSizeLookup() {
                    ...
                    override fun invalidateSpanIndexCache() {
                        ...
                        lookup.invalidateSpanGroupIndexCache()
                    }
                    ...
                    override fun getSpanIndex(position: Int, spanCount: Int): Int {
                        return lookup.getSpanIndex(position - startPosition, spanCount)
                    }

                    override fun getSpanGroupIndex(adapterPosition: Int, spanCount: Int): Int {
                        return lookup.getSpanGroupIndex(adapterPosition - startPosition, spanCount)
                    }
     ...
     */
    @Deprecated("强烈不建议使用", replaceWith = ReplaceWith("register{  }"))
    ...
    override fun onViewHolderCreate(parent: ViewGroup, viewType: Int): DuViewHolder<Any> {
        return MDuViewHolder(delegate.createView(parent, viewType)).also {
    ...
    fun onViewRecycled(view: View) {
        delegate.onViewRecycled(view)
    }

    inner class MDuViewHolder(containerView: View) : DuViewHolder<Any>(containerView) {
        override fun onBind(item: Any, position: Int) {
            bindView(containerView, item, position)
        }

        override fun onViewRecycled() {
            super.onViewRecycled()
            onViewRecycled(containerView)
        }
    }
}
