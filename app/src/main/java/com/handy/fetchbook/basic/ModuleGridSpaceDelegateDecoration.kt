...
internal class ModuleGridSpaceDelegateDecoration(private val moduleAdapter: IModuleAdapter) : RecyclerView.ItemDecoration() {

    private val spaceMap = mutableMapOf<String, MItemSpace>()

    fun copy(moduleAdapter: IModuleAdapter): ModuleGridSpaceDelegateDecoration {
        val decoration = ModuleGridSpaceDelegateDecoration(moduleAdapter)
    ...
    private fun getOffsets(
        ...
        if (false) {
            Log.d("Charles","getOffsets type:$groupType, groupPosition:$groupPosition, column:$column, left:${outRect.left}, right:${outRect.right}")
        }
    }
